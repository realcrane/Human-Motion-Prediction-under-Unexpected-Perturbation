...
def feed_vae_eval(pose_vae, ground_truth, condition, future_weights, device):
    ...
    z = torch.randn(len(condition), 64).to(device)
    #z = torch.zeros((1,32))
...
def main():
    ...
    with open('processed_data/train_data_low_dyn.pkl', 'rb') as f:
    ...
    mocap_data_ori_pos = mocap_data_ori[:, 3:30].reshape((-1, 9, 3))
    mocap_data_ori_vel = mocap_data_ori[:, 30:57].reshape((-1, 9, 3))
    mocap_data_ori_rot = mocap_data_ori[:, 57:].reshape((-1, 9, 6))
    ...
    with open('processed_data/test_data_low_dyn.pkl', 'rb') as f:
    ...
    test_max = test_mocap_data_ori.max(dim=0)[0]
    test_min = test_mocap_data_ori.min(dim=0)[0]
    test_avg = test_mocap_data_ori.mean(dim=0)
    test_std = test_mocap_data_ori.std(dim=0)

    # Make sure we don't divide by 0
    test_std[test_std == 0] = 1.0

    if args.norm_mode == "minmax":
        test_mocap_data = 2 * (test_mocap_data_ori - test_min) / (test_max - test_min) - 1
    ...
    test_mocap_data_ori_pos = test_mocap_data_ori[:, 3:30].reshape((-1, 9, 3))
    test_mocap_data_ori_vel = test_mocap_data_ori[:, 30:57].reshape((-1, 9, 3))
    test_mocap_data_ori_rot = test_mocap_data_ori[:, 57:].reshape((-1, 9, 6))
    ...
    test_all_indices = np.arange(test_batch_size)
    test_good_masks = np.isin(test_all_indices, test_bad_indices, assume_unique=True, invert=True)
    test_selectable_indices = test_all_indices[test_good_masks]
    ...
    for ep in range(1, args.num_epochs + 1):
        ...
        for num_mini_batch, indices in enumerate(sampler):
            ...
            for offset in range(args.num_steps_per_rollout):
                ...
                pred_vel = vae_output_dn[:, 30:57].reshape((-1, 9, 3))
        ...
        for i in range(num_peds):
            if i == 0:
                ...
                condition_range = torch.tensor(end_indices[i - 1]) + 1
            count_steps += num_steps - 1
            test_history[:, : args.num_condition_frames].copy_(mocap_data[condition_range])
            prediction = torch.zeros((num_steps, 111)).to(args.device)
            prediction[0] = copy.deepcopy(test_history.squeeze())
            for step in range(1, num_steps):
                prediction_range = (condition_range
                                    + torch.arange(step, step + 1).long()
                                    )
                ground_truth = mocap_data[prediction_range].reshape((1,1,-1))
                condition = test_history[:, : args.num_condition_frames]
                #vae_output, recon_loss = feed_vae_eval(pose_vae, ground_truth, condition, 1, args.device)
                (vae_output, _, _), (recon_loss, kl_loss) = feed_vae(
                    pose_vae, ground_truth, condition, future_weights
                )

                test_history = test_history.roll(1, dims=1)
                next_frame = vae_output[:, 0]
                test_history[:, 0].copy_(next_frame.cpu().detach())
        ...
        prediction_dn_all_vel = prediction_dn_all[:, 30:57].reshape((-1, 9, 3))
        ...
        test_sampler = BatchSampler(
            SubsetRandomSampler(test_selectable_indices),
            args.mini_batch_size,
            drop_last=True,
        )
        test_ep_recon_loss = 0
        test_ep_kl_loss = 0
        test_ep_recon_loss_phy_facing = 0
        test_ep_recon_loss_phy_pos = 0
        test_ep_recon_loss_phy_vel = 0
        test_ep_recon_loss_phy_rot = 0
        test_num_mini_batch = 1
        for test_num_mini_batch, test_indices in enumerate(test_sampler):
            test_t_indices = torch.LongTensor(test_indices)

            # condition is from newest...oldest, i.e. (t-1, t-2, ... t-n)
            test_condition_range = (
                    test_t_indices.repeat((args.num_condition_frames, 1)).t()
                    + torch.arange(args.num_condition_frames - 1, -1, -1).long()
            )

            test_t_indices += args.num_condition_frames
            history[:, : args.num_condition_frames].copy_(test_mocap_data[test_condition_range])

            for test_offset in range(args.num_steps_per_rollout):
                # dims: (num_parallel, num_window, feature_size)
                test_use_student = torch.rand(1) < sample_schedule[ep - 1]

                test_prediction_range = (
                        test_t_indices.repeat((args.num_future_predictions, 1)).t()
                        + torch.arange(test_offset, test_offset + args.num_future_predictions).long()
                )
                test_ground_truth = test_mocap_data[test_prediction_range]
                test_gt_facing = test_mocap_data_ori_facing[test_prediction_range].squeeze()
                test_gt_pos = test_mocap_data_ori_pos[test_prediction_range].squeeze()
                test_gt_vel = test_mocap_data_ori_vel[test_prediction_range].squeeze()
                test_gt_euler = test_euler[test_prediction_range].squeeze()
                test_condition = history[:, : args.num_condition_frames]
                ...
                history = history.roll(1, dims=1)
                test_next_frame = test_vae_output[:, 0] if test_use_student else test_ground_truth[:, 0]
                history[:, 0].copy_(test_next_frame.detach())


                test_ep_recon_loss += float(test_recon_loss) / args.num_steps_per_rollout
                test_ep_kl_loss += float(test_kl_loss) / args.num_steps_per_rollout

                test_vae_output_dn = pose_vae.denormalize(test_vae_output.detach()[:, 0])
                test_pred_facing = test_vae_output_dn[:, 2]
                test_pred_pos = test_vae_output_dn[:, 3:30].reshape((-1, 9, 3))
                test_pred_vel = test_vae_output_dn[:, 30:57].reshape((-1, 9, 3))
                test_pred_rot = test_vae_output_dn[:, 57:].reshape((-1, 9, 6))
                test_pred_rm = from_6D_to_rm(test_pred_rot).cpu().detach().numpy()
                test_pred_euler = np.zeros((len(test_pred_rm), 9, 3))
                for joint in range(9):
                    r = Rotation.from_matrix(test_pred_rm[:, joint])
                    test_pred_euler[:, joint] = r.as_euler(bvh_order, degrees=True)
                test_pred_euler = torch.from_numpy(test_pred_euler)

                test_ep_recon_loss_phy_facing += torch.abs(test_gt_facing - test_pred_facing).mean() / args.num_steps_per_rollout
                test_ep_recon_loss_phy_pos += torch.norm(test_gt_pos - test_pred_pos, dim=-1).mean() / args.num_steps_per_rollout
                test_ep_recon_loss_phy_vel += torch.norm(test_gt_vel - test_pred_vel, dim=-1).mean() / args.num_steps_per_rollout
                test_ep_recon_loss_phy_rot += torch.abs(test_gt_euler - test_pred_euler).mean() / args.num_steps_per_rollout

        test_avg_ep_recon_loss = test_ep_recon_loss / test_num_mini_batch
        test_avg_ep_kl_loss = test_ep_kl_loss / test_num_mini_batch

        test_avg_ep_recon_loss_phy_facing = test_ep_recon_loss_phy_facing / test_num_mini_batch
        test_avg_ep_recon_loss_phy_pos = test_ep_recon_loss_phy_pos / test_num_mini_batch
        test_avg_ep_recon_loss_phy_vel = test_ep_recon_loss_phy_vel / test_num_mini_batch
        test_avg_ep_recon_loss_phy_rot = test_ep_recon_loss_phy_rot / test_num_mini_batch

        logger_curve.add_scalar('test/recon_loss_1', test_avg_ep_recon_loss, ep)
        logger_curve.add_scalar('test/kl_loss', test_avg_ep_kl_loss, ep)
        logger_curve.add_scalar('test/recon_loss_1_facing', test_avg_ep_recon_loss_phy_facing, ep)
        logger_curve.add_scalar('test/recon_loss_1_pos', test_avg_ep_recon_loss_phy_pos / 1000, ep)
        logger_curve.add_scalar('test/recon_loss_1_vel', test_avg_ep_recon_loss_phy_vel / 1000, ep)
        logger_curve.add_scalar('test/recon_loss_1_rot', test_avg_ep_recon_loss_phy_rot, ep)
        ...
        for i in range(test_num_peds):
            ...
            for step in range(1, test_num_steps):
                ...
                test_ground_truth = test_mocap_data[test_prediction_range].reshape((1,1,-1))
        ...
        test_prediction_dn_all_vel = test_prediction_dn_all[:, 30:57].reshape((-1, 9, 3))
